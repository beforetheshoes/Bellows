                ...
                VStack(alignment: .leading, spacing: DS.Metrics.spacing) {
                        ...
                        VStack(spacing: 12) {
                            HStack {
                                ...
                                Picker("", selection: $selectedExercise) {
                                    ...
                                    Divider()
                                    ForEach(filteredExerciseTypes) { exercise in Text(exercise.name).tag(Optional(exercise)) }
                                }
                                .pickerStyle(.menu)
                                .labelsHidden()
                            }
                            ...
                            HStack {
                                ...
                                Spacer()
                                HStack {
                                    Text(amountOnlyString(amount, unit: selectedUnit)).monospacedDigit().fontWeight(.medium)
                                    Stepper("", value: $amount, in: 0...10_000, step: stepForUnit(selectedUnit)).labelsHidden()
                                }
                            }
                        }
                    }
                    ...
                    SectionCard {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            TextField("Optional", text: $note, axis: .vertical).font(.body).textFieldStyle(.roundedBorder)
                        }
                    }
                }
